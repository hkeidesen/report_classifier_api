...
def get_all_pdf_link_on_url(pages):
    ...
    # columns = ['URLs to PDFs']
    # df = pd.DataFrame(columns=columns)
    # df = df.append(url_to_reports)
...
def find_url_to_all_reportpages():
    ...
    for i in range(1,end_page):
        ...
        df = df.append(get_all_pdf_link_on_url(url))
        df.to_excel('urls.xlsx')
...
def main(report_url):
    ...
    deviation_columns = ['Avviksnummer',
                'Tittel på avvik',
                'Avvikets beskrivende tekst',
                'Alle regelhenvisninger (avvik)', 
                'Kategori (avvik)']
    ...
    improvement_columns = ['Forbedringspunkter',
                    'Tittel på forbedringspunkt',
                    'Forbedringens beskrivende tekst',
                    'Alle regelhenvisninger (forbedring)', 
                    'Kategori (forbedringer)']
